...
int do_hash_db_test()
{
    ...
    assert(db.Write<int>(HashDB::Hash("value3"), 44) == 0);

    int read_value[3] = { 0 };
    assert(!db.Read<int>(HashDB::Hash("value1"), read_value[0]));
    assert(!db.Read<int>(HashDB::Hash("value2"), read_value[1]));
    assert(!db.Read<int>(HashDB::Hash("value3"), read_value[2]));
    ...
    assert(!db.Write<int>(HashDB::Hash("value1"), 52));
    std::cout << "Used size: " << db.GetUsedSize() << " / "
              << db.GetMaxSize() << std::endl;
    assert(!db.Write<int>(HashDB::Hash("value2"), 53));
    std::cout << "Used size: " << db.GetUsedSize() << " / "
              << db.GetMaxSize() << std::endl;
    assert(!db.Write<int>(HashDB::Hash("value3"), 54));
    std::cout << "Used size: " << db.GetUsedSize() << " / "
              << db.GetMaxSize() << std::endl;
    assert(!db.Read<int>(HashDB::Hash("value1"), read_value[0]));
    assert(!db.Read<int>(HashDB::Hash("value2"), read_value[1]));
    assert(!db.Read<int>(HashDB::Hash("value1"), read_value[2]));
    std::cout << "Read from HashDB: " << read_value[0] << std::endl;
    std::cout << "Read from HashDB: " << read_value[1] << std::endl;
    std::cout << "Read from HashDB: " << read_value[2] << std::endl;
    ...
    assert(!db.Write<somedata>(HashDB::Hash("data1"), data));  
    somedata read_data;
    assert(!db.Read<somedata>(HashDB::Hash("data1"), read_data));
    std::cout << "Read from HashDB: " << read_data.buf << ", "
              << (uint32_t)read_data.val << std::endl;

    assert(db.Read<somedata>(HashDB::Hash("data1"), read_data) == 0);
    std::cout << "Used size: " << db.GetUsedSize() << " / "
              << db.GetMaxSize() << std::endl;
