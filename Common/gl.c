...
#if (!defined(LCD_MAXX) || !defined(LCD_MAXY))
#define LCD_MAXX	240
#define LCD_MAXY	320
#endif

static const float sin_table[256] = { 0.00000,0.02454,0.04907,0.07356,0.09802,0.12241,0.14673,0.17096,
...
static uint16_t gl_buffer[LCD_MAXY + 1];
static uint16_t *bgbuffer = gl_buffer;
static int16_t zbuf[LCD_MAXY + 1];
static uint16_t gl_x = 1;

static float glmat[16];
static const struct glVector3 light = {.x=1,.y=0,.z=0,.col=0};
static struct glPoint glPoints[GL_POINT_MAX];
static uint8_t glPoint_num=0;
static struct glLine glLines[GL_LINE_MAX];
static uint8_t glLine_num=0;
static struct glTriangle glTriangles[GL_TRIANGLE_MAX];
static uint16_t glTriangle_num=0;
static const float M_PI = 3.1415926;

extern uint16_t rgb2col(uint8_t r, uint8_t g, uint8_t b);
...
void glIntTransform(const struct glVector3 *src,
	struct glVector3int *res)
{
		...
		const struct glVector3 *t3)
{
	...
	t->col = t1->col;
	if (glTriangle_num<GL_TRIANGLE_MAX)
		glTriangle_num++;
}
...
static void glIntDraw()
{
	uint16_t i;
	glIntClear(0);

	for (i=0;i<glPoint_num;i++)
		glIntDrawPoint(&glPoints[i]);

	for (i=0;i<glLine_num;i++)
		glIntDrawLine(&glLines[i]);

	for (i=0;i<glTriangle_num;i++)
		glIntDrawTriangle(&glTriangles[i]);
}
	...
	uint16_t y, uint16_t dx, uint16_t dy, uint16_t *pic))
{
	...
	for (gl_x = 0; gl_x < LCD_MAXX; gl_x++) {
		glIntDraw();
		draw_pic(gl_x, 0, 1, LCD_MAXY,
			gl_buffer);
	}
	glPoint_num = 0;
	glLine_num = 0;
